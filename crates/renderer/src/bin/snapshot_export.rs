...
fn main() {
    ...
    let (passed, failed, new) = figma_renderer::verify::run_all_tests();

    // Also run each test individually to get pixel data
    let tests = figma_renderer::verify::get_all_tests();
    for test in &tests {
        let (pixels, hash) = figma_renderer::verify::render_test_pixels(test);
