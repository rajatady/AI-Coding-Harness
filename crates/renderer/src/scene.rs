...
pub enum RenderShape {
    Rect {
        width: f32,
        height: f32,
        corner_radii: figma_engine::node::CornerRadii,
    },
    Ellipse {
        width: f32,
        height: f32,
    ...
    Path {
        commands: Vec<figma_engine::node::PathCommand>,
        fill_rule: FillRule,
    },
    Text {
        runs: Vec<figma_engine::node::TextRun>,
        width: f32,
        height: f32,
        align: figma_engine::node::TextAlign,
...
fn build_scene_recursive(
    ...
    let (shape, clips) = match &node.kind {
        ...
        NodeKind::BooleanOp { .. } => {
            ...
            if let Some(result) = figma_engine::boolean::compute_boolean(tree, node_id) {
