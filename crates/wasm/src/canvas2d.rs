...
pub fn render_items_with_camera(
    ...
    zoom: f64,
    dpr: f64,
    text_arc_params: &std::collections::HashMap<figma_engine::id::NodeId, (f32, f32, f32)>,
) -> usize {
...
fn render_item_range(
    ...
    while i < end {
        ...
        if !item.style.strokes.is_empty() && item.style.stroke_weight > 0.0 {
            let is_inside = matches!(item.style.stroke_align, figma_engine::properties::StrokeAlign::Inside);
            let is_outside = matches!(item.style.stroke_align, figma_engine::properties::StrokeAlign::Outside);
...
fn draw_text(
    ...
    align: TextAlign,
    vertical_align: figma_engine::node::TextVerticalAlign,
    width: f64,
    height: f64,
) {
    use figma_engine::node::{TextDecoration, TextVerticalAlign};
...
fn draw_text_decoration(
    ...
    y: f64,
    font_size: f64,
    decoration: figma_engine::node::TextDecoration,
) {
