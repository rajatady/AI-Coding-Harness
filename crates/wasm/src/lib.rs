...
use figma_renderer::pipeline;
use figma_renderer::scene::{AABB, RenderItem};
use glam::Vec2;
...
impl FigmaApp {
    ...
    fn scene_insert_leaf(&mut self, node: &figma_engine::node::Node, parent_id: NodeId) {
        ...
        let (shape, clips) = match &node.kind {
            figma_engine::node::NodeKind::Rectangle { corner_radii } => (
                figma_renderer::scene::RenderShape::Rect {
                    width: node.width, height: node.height,
                    corner_radii: *corner_radii,
                }, false
            ),
            figma_engine::node::NodeKind::Ellipse { arc_start, arc_end, inner_radius_ratio } => (
                figma_renderer::scene::RenderShape::Ellipse {
                    width: node.width, height: node.height,
                    arc_start: *arc_start, arc_end: *arc_end,
                    inner_radius_ratio: *inner_radius_ratio,
                }, false
            ),
            figma_engine::node::NodeKind::Text { runs, align, vertical_align, .. } => (
                figma_renderer::scene::RenderShape::Text {
                    runs: runs.clone(), width: node.width, height: node.height,
                    align: *align, vertical_align: *vertical_align,
                }, false
            ),
            figma_engine::node::NodeKind::Frame { clip_content, corner_radii, .. } => (
                figma_renderer::scene::RenderShape::Rect {
                    width: node.width, height: node.height,
                    corner_radii: *corner_radii,
                }, *clip_content
            ),
            figma_engine::node::NodeKind::Line => (
                figma_renderer::scene::RenderShape::Line { length: node.width }, false
            ),
            _ => (
                figma_renderer::scene::RenderShape::Rect {
                    width: node.width, height: node.height,
    ...
    fn apply_constraints(&mut self, parent_id: NodeId, old_w: f32, old_h: f32, new_w: f32, new_h: f32) {
        use figma_engine::properties::ConstraintType;
        let dw = new_w - old_w;
        let dh = new_h - old_h;
    ...
    fn commit_pen_path(&mut self, closed: bool) {
        use figma_engine::node::{PathCommand, VectorPath, NodeKind};
        ...
        let mut node = Node {
            ...
            width,
            height,
            style: figma_engine::properties::Style::default(),
            ...
            InteractionMode::Resizing { node_id, orig_w, orig_h, orig_tx, orig_ty, .. } => {
                ...
                if let Some((w, h, t)) = props {
                    ...
                    self.pending_ops.push(Operation {
                        ...
                        kind: OpKind::SetProperty {
                            node_id,
                            property: figma_crdt::operation::PropertyUpdate::Width(w),
                        },
                    });
                    ...
                    self.pending_ops.push(Operation {
                        ...
                        kind: OpKind::SetProperty {
                            node_id,
                            property: figma_crdt::operation::PropertyUpdate::Height(h),
                        },
                    });
    ...
    pub fn export_pixels(&self, width: u32, height: u32) -> Vec<u8> {
        ...
        let raw_items = figma_renderer::scene::build_scene(&page.tree, &root_id, &viewport);
    ...
    pub fn get_node_info(&self, counter: u32, client_id: u32) -> String {
        ...
        let blend_str = match node.style.blend_mode {
            figma_engine::properties::BlendMode::Normal => "normal",
            figma_engine::properties::BlendMode::Multiply => "multiply",
            figma_engine::properties::BlendMode::Screen => "screen",
            figma_engine::properties::BlendMode::Overlay => "overlay",
            figma_engine::properties::BlendMode::Darken => "darken",
            figma_engine::properties::BlendMode::Lighten => "lighten",
            figma_engine::properties::BlendMode::ColorDodge => "color-dodge",
            figma_engine::properties::BlendMode::ColorBurn => "color-burn",
            figma_engine::properties::BlendMode::HardLight => "hard-light",
            figma_engine::properties::BlendMode::SoftLight => "soft-light",
            figma_engine::properties::BlendMode::Difference => "difference",
            figma_engine::properties::BlendMode::Exclusion => "exclusion",
            figma_engine::properties::BlendMode::Hue => "hue",
            figma_engine::properties::BlendMode::Saturation => "saturation",
            figma_engine::properties::BlendMode::ColorMode => "color",
            figma_engine::properties::BlendMode::Luminosity => "luminosity",
        };
        ...
        let ch_str = match node.constraint_horizontal {
            figma_engine::properties::ConstraintType::Min => "left",
            figma_engine::properties::ConstraintType::Max => "right",
            figma_engine::properties::ConstraintType::MinMax => "leftRight",
            figma_engine::properties::ConstraintType::Center => "center",
            figma_engine::properties::ConstraintType::Scale => "scale",
        };
        let cv_str = match node.constraint_vertical {
            figma_engine::properties::ConstraintType::Min => "top",
            figma_engine::properties::ConstraintType::Max => "bottom",
            figma_engine::properties::ConstraintType::MinMax => "topBottom",
            figma_engine::properties::ConstraintType::Center => "center",
            figma_engine::properties::ConstraintType::Scale => "scale",
        };
        ...
        let auto_layout_json = if let NodeKind::Frame { auto_layout: Some(al), .. } = &node.kind {
            let dir = match al.direction {
                figma_engine::properties::LayoutDirection::Horizontal => "horizontal",
                figma_engine::properties::LayoutDirection::Vertical => "vertical",
            };
            format!(
    ...
    pub fn get_tree_layers(&self, expanded_ids: &str, start: u32, count: u32) -> Vec<u32> {
        ...
        fn walk(
            tree: &figma_engine::tree::DocumentTree,
            node_id: &NodeId,
            depth: u16,
            ...
            let kind = tree.get(node_id).map(|n| match &n.kind {
                figma_engine::node::NodeKind::Frame { .. } => 0u8,
                figma_engine::node::NodeKind::Rectangle { .. } => 1,
                figma_engine::node::NodeKind::Ellipse { .. } => 2,
                figma_engine::node::NodeKind::Text { .. } => 3,
                figma_engine::node::NodeKind::Vector { .. } => 4,
                figma_engine::node::NodeKind::Image { .. } => 5,
                figma_engine::node::NodeKind::BooleanOp { .. } => 6,
                _ => 7,
            }).unwrap_or(7);
    ...
    pub fn set_node_corner_radius(
        ...
        use figma_engine::node::CornerRadii;
        let node_id = NodeId(figma_engine::id::LogicalClock { counter: counter as u64, client_id });
        let page = match self.document.page_mut(self.current_page) {
            Some(p) => p,
            None => return false,
    ...
    pub fn set_node_blend_mode(&mut self, counter: u32, client_id: u32, mode: u32) -> bool {
        use figma_engine::properties::BlendMode;
    ...
    pub fn set_auto_layout(
        ...
        use figma_engine::properties::{AutoLayout, LayoutDirection, SizingMode, LayoutAlign};
        let node_id = NodeId(figma_engine::id::LogicalClock { counter: counter as u64, client_id });
        let page = match self.document.page_mut(self.current_page) {
            Some(p) => p,
            None => return false,
