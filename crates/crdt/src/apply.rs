...
fn apply_property(node: &mut figma_engine::node::Node, prop: &PropertyUpdate) {
    match prop {
        ...
        PropertyUpdate::ClipContent(clip) => {
            if let figma_engine::node::NodeKind::Frame { clip_content, .. } = &mut node.kind {
                *clip_content = *clip;
            }
        }
        PropertyUpdate::AutoLayout(al) => {
            if let figma_engine::node::NodeKind::Frame { auto_layout, .. } = &mut node.kind {
                *auto_layout = al.clone();
            }
        }
        PropertyUpdate::TextRuns(runs) => {
            if let figma_engine::node::NodeKind::Text { runs: existing, .. } = &mut node.kind {
                *existing = runs.clone();
            }
        }
        PropertyUpdate::TextAlign(align) => {
            if let figma_engine::node::NodeKind::Text { align: existing, .. } = &mut node.kind {
                *existing = *align;
            }
        }
    }
