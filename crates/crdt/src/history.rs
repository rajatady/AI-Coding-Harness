...
fn get_current_property(
    ...
    match property {
        ...
        PropertyUpdate::CornerRadii(_) => {
            match &node.kind {
                figma_engine::node::NodeKind::Frame { corner_radii, .. }
                | figma_engine::node::NodeKind::Rectangle { corner_radii } => {
                    PropertyUpdate::CornerRadii(*corner_radii)
                }
                _ => PropertyUpdate::CornerRadii(figma_engine::node::CornerRadii::default()),
            }
        }
        PropertyUpdate::ClipContent(_) => {
            if let figma_engine::node::NodeKind::Frame { clip_content, .. } = &node.kind {
        ...
        PropertyUpdate::AutoLayout(_) => {
            if let figma_engine::node::NodeKind::Frame { auto_layout, .. } = &node.kind {
        ...
        PropertyUpdate::TextRuns(_) => {
            if let figma_engine::node::NodeKind::Text { runs, .. } = &node.kind {
        ...
        PropertyUpdate::TextAlign(_) => {
            if let figma_engine::node::NodeKind::Text { align, .. } = &node.kind {
                PropertyUpdate::TextAlign(*align)
            } else {
                PropertyUpdate::TextAlign(figma_engine::node::TextAlign::Left)
            }
        }
    }
}
